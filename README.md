# Kiran Shah
#### email: kiran@kdnshah.com
### Data Scientist | Sports Analytics | Machine Learning | Bayesian Modeling

MS Analytics, Georgia Tech · Analytics at DC United (MLS)

---

## About

I am a data scientist and sports analytics practitioner with a background in finance and an M.S. in Analytics from Georgia Tech. My work focuses on transforming complex data into actionable insights through machine learning, statistical modeling, optimization, and visualization.

I have developed analytics projects for organizations including DC United, the United States Olympic & Paralympic Committee (USOPC), and Houston Dynamo FC, with experience spanning player valuation, recruitment analysis, performance forecasting, and decision support systems.

---

## Featured Projects

### [Bayesian Possession Value Model](https://github.com/kshah-340/bayesian-possession-value)
A Bayesian framework for estimating how individual actions contribute to possession success in soccer. Built using StatsBomb event data and hierarchical Bayesian modeling techniques.

**Skills**: Bayesian Statistics, PyMC, Soccer Analytics, Feature Engineering, Data Visualization

🔗 [GitHub](https://github.com/kshah-340/bayesian-possession-value) · [Medium Writeup](https://medium.com/@kiran_30122/a-bayesian-approach-to-quantifying-soccer-possession-value-a7d17435bf35)

---

### [Houston Dynamo FC Striker Recruitment Analysis](https://github.com/kshah-340/dynamo_striker_recruitment)

Developed a data-driven recruitment framework integrating Wyscout and SkillCorner data to identify striker profiles that fit a desired tactical system.

**Skills**: Player Recruitment, Statistical Analysis, Soccer Analytics, Tableau, Python

---

### [NFL Offensive Information Leakage](https://github.com/kshah-340/nfl_offensive_information_leakage)

A research project theorizing, computing, and evaluating the tradeoff between play-call predictability and play effectiveness. Inspired by concepts from financial market theory, information economics, and game theory.

**Skills**: Machine Learning, NFL Analytics, Logistic Regression, Model Evaluation, Feature Engineering

---

### [Olympic Medal Forecasting and Interactive Dashboard](repository link coming soon)

Built Bayesian mixed-effects performance forecasting models and interactive dashboards for speed skating competition analysis.

**Skills**: Bayesian Inference, Machine Learning, Sports Analytics, Mixed-Effects Regression, Tableau Visualization, Python

---

## Technical Skills

### Programming
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=flat&logo=r&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

### Machine Learning and Statistics
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-007ACC?style=flat)
![Bayesian Statistics](https://img.shields.io/badge/Bayesian%20Statistics-2D6A4F?style=flat)
![PyMC](https://img.shields.io/badge/PyMC-FF6F00?style=flat)
![PyCaret](https://img.shields.io/badge/PyCaret-FFD43B?style=flat&logo=python&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![Regression](https://img.shields.io/badge/Regression-00599C?style=flat)
![Classification](https://img.shields.io/badge/Classification-6A1B9A?style=flat)
![Optimization](https://img.shields.io/badge/Optimization-C62828?style=flat)

### Analytics and Visualization
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=flat&logo=plotly&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat)
![JMP](https://img.shields.io/badge/JMP%20Pro-0066CC?style=flat)

### Data Engineering and Tools
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat&logo=amazonaws&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat&logo=databricks&logoColor=white)
![GCP](https://img.shields.io/badge/-Google%20Cloud%20Platform-4285F4?style=flat&logo=google%20cloud&logoColor=white)

---

## Professional Experience

### DC United - Strategy and Analytics

Develop player valuation methodologies and decision-support tools for football operations.

### Bank of America / Merrill Lynch

Financial Solutions Advisor responsible for investment analysis, client portfolio guidance, and strategic financial planning.

---

## Education

### Georgia Institute of Technology

M.S. Analytics

### University of Florida

B.S. Finance, Magna Cum Laude

---

## Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kiran-shah-340ax/)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:kiran@kdnshah.com)
[![Medium](https://img.shields.io/badge/Medium-12100E?style=flat&logo=medium&logoColor=white)](https://medium.com/@kiran_30122/a-bayesian-approach-to-quantifying-soccer-possession-value-a7d17435bf35)
